...
<xsl:variable name="authorlabel">作者：</xsl:variable>
<xsl:variable name="translatorlabel">翻译：</xsl:variable>
...
<xsl:template name="user.head.content">
  ...
  <meta name="generator" content="Experimental LDP.XSL $Revision: 1.2 $"/>
  <xsl:text>
  </xsl:text>
...
<xsl:template match="author" mode="titlepage.mode">
  ...
  <xsl:choose>
    ...
    <xsl:when test="@role='translator'">
		<i><xsl:value-of select="$translatorlabel"/></i>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template name="user.footer.content">
	<script type="text/javascript">
	  ...
	  var _gaq = _gaq || [];
	  _gaq.push(['_setAccount', 'UA-10358505-6']);
	  _gaq.push(['_setDomainName', '.lvhai.org']);
